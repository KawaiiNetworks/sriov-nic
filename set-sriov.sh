...
finish_interactive_setup() {
    ...
    if [[ "$absolute_target" == "$SCRIPT_DIR"/sriov-nic.conf* ]]; then
        if prompt_yes_no "Install/update and enable sriov-nic.service for this config?" no; then
            ...
            if [[ "$MODE" == switchdev ]] &&
               prompt_yes_no "Use Open vSwitch for this PF/its representors?" no; then
                command_exists ovs-vsctl || \
                    die "Open vSwitch was selected, but ovs-vsctl is not installed."
                OVS_HW_OFFLOAD=true
            fi
        fi
    fi
